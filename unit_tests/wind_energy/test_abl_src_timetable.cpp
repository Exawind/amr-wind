...
TEST_F(ABLSrcTimeTableTest, geostrophic)
{
    ...
    const amrex::Real cf =
        2.0_rt *
        (2.0_rt * static_cast<amrex::Real>(std::numbers::pi) / 86164.091_rt) *
        std::sin(
            static_cast<amrex::Real>(std::numbers::pi) / 180.0_rt * 90.0_rt);
    ...
    const amrex::Vector<amrex::Real> targ_vel{
        8.0_rt * std::cos(
                     static_cast<amrex::Real>(std::numbers::pi) / 180.0_rt *
                     1.25_rt),
        8.0_rt * std::sin(
                     static_cast<amrex::Real>(std::numbers::pi) / 180.0_rt *
                     1.25_rt),
