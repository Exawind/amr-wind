...
TEST_F(IBDiracDeltaTest, ib_utils)
{
    ...
    fill_lagrangian_markers(
        lag_markers, 11, geom[0].ProbLoArray(), geom[0].ProbHiArray());
                ...
                amrex::ParallelFor(
                    bx, [&] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                        ...
                        const auto dirac_delta_fac =
                            amr_wind::ib::utils::dirac_delta(
                                dist_x, {dx[0], dx[1], dx[2]});

                        sum1 += dirac_delta_fac * dx[0] * dx[1] * dx[2];
                        sum2 +=
                            dist_x[0] * dirac_delta_fac * dx[0] * dx[1] * dx[2];
                        sum3 +=
                            dist_x[1] * dirac_delta_fac * dx[0] * dx[1] * dx[2];
                        sum4 +=
                            dist_x[2] * dirac_delta_fac * dx[0] * dx[1] * dx[2];
                    });
            }
        }
