...
void incflo::ApplyPredictor(bool incremental_projection)
{
    ...
    if (sim().has_overset()) {
        // Sharpen nalu fields if multiphase simulation
        if (sim().is_multiphase()) {
    ...
    if (sim().has_overset() && sim().is_multiphase()) {
        // Replace pressure gradient to get original velocity
