...
x_geom(const amrex::Real x, const amrex::Real sratio, const amrex::Real delta0)
{
    return delta0 / sratio / (1.0 - sratio) *
               (1.0 - std::pow(sratio, x + 1.0)) -
           delta0 / sratio;
}
    ...
    const amrex::Real delta0)
{
    return int(
        std::log(
            1.0 -
            (trans_loc - delta0 / sratio) * sratio * (1.0 - sratio) / delta0) /
        std::log(sratio));
}
    ...
    const int domap)
{
    ...
    const amrex::Real h =
        0.5 * (1.0 + std::tanh((x * dxscale - xinv) / trans_width));
    const amrex::Real hprime =
        0.5 * std::pow(std::cosh((x * dxscale - xinv) / trans_width), -2.0) /
        trans_width;
    const amrex::Real lmatch = x_geom(xinv, sratio, delta0);
    const amrex::Real slope = lmatch - x_geom(xinv - 1.0, sratio, delta0);
    const amrex::Real xprime_geom = delta0 / sratio / (sratio - 1.0) *
                                    std::pow(sratio, x * dxscale - 1.0) *
                                    std::log(sratio);

    if (domap == 0) return 1.0;
    return ((1.0 - h) * xprime_geom -
            hprime * x_geom(x * dxscale, sratio, delta0) + h * slope +
            hprime * x_const(x * dxscale, slope, xinv, lmatch)) *
           dxscale;
}
    ...
    const int domap)
{
    ...
    if (domap == 0) return x;
    return x_geom(x * dxscale, sratio, delta0) * (1.0 - h) +
           h * x_const(x * dxscale, slope, xinv, lmatch);
}
    ...
    int lev, const amrex::Geometry& geom)
{
    ...
    amrex::Print() << "Transition Width:    " << transwid[0] << " "
                   << transwid[1] << " " << transwid[2] << std::endl;

    amrex::Print() << "Transition Location:    " << transloc[0] << " "
                   << transloc[1] << " " << transloc[2] << std::endl;
        ...
        amrex::ParallelFor(
            bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                ...
                amrex::Real fac_x = eval_fac(
                    x, transloc[0], transwid[0], sratio[0], delta0[0],
                    dxscale[0], do_map[0]);
                amrex::Real fac_y = eval_fac(
                    y, transloc[1], transwid[1], sratio[1], delta0[1],
                    dxscale[1], do_map[1]);
                amrex::Real fac_z = eval_fac(
                    z, transloc[2], transwid[2], sratio[2], delta0[2],
                    dxscale[2], do_map[2]);

                bool x_below = x < prob_lo[0];
                bool y_below = y < prob_lo[1];
                bool z_below = z < prob_lo[2];
                const amrex::Real fac_x_below =
                    do_map[0] ? delta0[0] * dxscale[0] : 1.0;
                const amrex::Real fac_y_below =
                    do_map[1] ? delta0[1] * dxscale[1] : 1.0;
                const amrex::Real fac_z_below =
                    do_map[2] ? delta0[2] * dxscale[2] : 1.0;
    ...
    for (amrex::MFIter mfi((*m_non_uniform_coord_cc)(lev)); mfi.isValid();
        ...
        amrex::ParallelFor(
            bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                ...
                bool y_below = y < prob_lo[1];
                bool z_below = z < prob_lo[2];
                const amrex::Real cc_x_below =
                    do_map[0] ? prob_lo[0] + (i + 0.5) * delta0[0] : x;
                const amrex::Real cc_y_below =
                    do_map[1] ? prob_lo[1] + (j + 0.5) * delta0[1] : y;
                const amrex::Real cc_z_below =
                    do_map[2] ? prob_lo[2] + (k + 0.5) * delta0[2] : z;
        ...
        amrex::ParallelFor(
            nbx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                ...
                bool y_below = y < prob_lo[1];
                bool z_below = z < prob_lo[2];
                const amrex::Real nd_x_below =
                    do_map[0] ? prob_lo[0] + i * delta0[0] : x;
                const amrex::Real nd_y_below =
                    do_map[1] ? prob_lo[1] + j * delta0[1] : y;
                const amrex::Real nd_z_below =
                    do_map[2] ? prob_lo[2] + k * delta0[2] : z;
