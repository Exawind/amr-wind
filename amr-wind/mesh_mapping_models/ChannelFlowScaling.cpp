            ...
            bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                ...
                amrex::Real num_x =
                    -(2 * m_beta[0] * ht[0] *
                      ...
                      std::pow(
                          ((m_beta[0] + 1) / (m_beta[0] - 1)),
                          ((m_alpha[0] + x) / (1 - m_alpha[0]))));
                amrex::Real num_y =
                    -(2 * m_beta[1] * ht[1] *
                      log((m_beta[1] + 1) / (m_beta[1] - 1)) *
                      std::pow(
                          ((m_beta[1] + 1) / (m_beta[1] - 1)),
                          ((m_alpha[1] + y) / (1 - m_alpha[1]))));
                amrex::Real num_z =
                    -(2 * m_beta[2] * ht[2] *
                      log((m_beta[2] + 1) / (m_beta[2] - 1)) *
                      std::pow(
                          ((m_beta[2] + 1) / (m_beta[2] - 1)),
                          ((m_alpha[2] + z) / (1 - m_alpha[2]))));

                amrex::Real den_x =
                    ((m_alpha[0] - 1) * (2 * m_alpha[0] + 1) *
                     std::pow(
                         (std::pow(
                              ((m_beta[0] + 1) / (m_beta[0] - 1)),
                              (x / (1 - m_alpha[0]))) +
                          std::pow(
                              ((m_beta[0] + 1) / (m_beta[0] - 1)),
                              (m_alpha[0] / (1 - m_alpha[0])))),
                         2));
                amrex::Real den_y =
                    ((m_alpha[1] - 1) * (2 * m_alpha[1] + 1) *
                     std::pow(
                         (std::pow(
                              ((m_beta[1] + 1) / (m_beta[1] - 1)),
                              (y / (1 - m_alpha[1]))) +
                          std::pow(
                              ((m_beta[1] + 1) / (m_beta[1] - 1)),
                              (m_alpha[1] / (1 - m_alpha[1])))),
                         2));
                amrex::Real den_z =
                    ((m_alpha[2] - 1) * (2 * m_alpha[2] + 1) *
                     std::pow(
                         (std::pow(
                              ((m_beta[2] + 1) / (m_beta[2] - 1)),
                              (z / (1 - m_alpha[2]))) +
                          std::pow(
                              ((m_beta[2] + 1) / (m_beta[2] - 1)),
                              (m_alpha[2] / (1 - m_alpha[2])))),
                         2));

                scale_fac_cc(i, j, k, 0) =
                    m_map[0] ? num_x / den_x / dx[0] : 1.0;
                scale_fac_cc(i, j, k, 1) =
                    m_map[1] ? num_y / den_y / dx[1] : 1.0;
                scale_fac_cc(i, j, k, 2) =
                    m_map[2] ? num_z / den_z / dx[2] : 1.0;
            });
