...
amrex::Real MOData::phi_m() const { return phi_m(zref); }
...
amrex::Real MOData::phi_h() const { return phi_h(zref); }
...
amrex::Real MOData::psi_m(amrex::Real z) const
{
    ...
    // if unstable:
        ...
        psi_m = 2.0 * std::log((1.0 + std::pow(phi_m(z), -1.0)) / 2.0) +
                ...
                2.0 * std::atan(1.0 / phi_m(z)) + utils::half_pi();
    }
...
amrex::Real MOData::psi_h(amrex::Real z) const
{
    ...
    // if unstable:
        ...
        psi_h =
            2.0 * std::log((1.0 + std::pow(phi_h(z) / alpha_h, -1.0)) / 2.0);
    }
...
{
    utau =
        (kappa * vmag_mean) / (std::log(zref / z0) - (psi_m(zref) - psi_m(z0)));
...
{
    surf_temp =
        theta_mean + ((alpha_h * surf_temp_flux) / (kappa * utau)) *
                         (std::log(zref / z0) - (psi_h(zref) - psi_h(z0)));
}
...
{
    surf_temp_flux =
        (-(theta_mean - surf_temp) * kappa * utau) /
...
{
    amrex::Real utau1 =
        std::max(1.0E-6, (kappa * vmag_mean) / (std::log(zref / z0)));
    ...
    while ((std::abs(utau1 - utau0) > tol) && (iter < max_iters)) {
        ...
        /*
                ...
                std::cout << "        surf_temp_flux = " << surf_temp_flux <<
           std::endl; std::cout << "        surf_temp = " << surf_temp <<
           std::endl; std::cout << "        phi_m = " << phi_m() << std::endl;
