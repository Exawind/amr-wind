        ...
        if (has_terrain) {
            ...
            amrex::ParallelFor(
                ...
                [=] AMREX_GPU_DEVICE(int nbx, int i, int j, int k) {
                    ...
                    amrex::Real Rt =
                        utils::pow(tke_arrs[nbx](i, j, k) / epsilon, 2) *
                        stratification;
                    Rt = (Rt > Rtc)
                             ? Rt
                             : amrex::max<amrex::Real>(
                                   Rt, Rt - (utils::pow(Rt - Rtc, 2) /
                                             (Rt + Rtmin - 2.0_rt * Rtc)));
                    tlscale_arrs[nbx](i, j, k) =
                        (stratification > 0)
                            ? std::sqrt(
                                  utils::pow(lscale_s * lscale_b, 2) /
                                  (utils::pow(lscale_s, 2) +
                                   utils::pow(lscale_b, 2)))
                            : lscale_s *
                                  std::sqrt(
                                      1.0_rt -
                                      (utils::pow(Cmu, 6) *
                                       utils::pow(Cb_unstable, -2) * Rt));
                    tlscale_arrs[nbx](i, j, k) =
                        (stratification > 0)
            ...
            mu_turb(lev), [=] AMREX_GPU_DEVICE(int nbx, int i, int j, int k) {
                amrex::Real stratification =
                    -((gradT_arrs[nbx](i, j, k, 0) * gravity[0]) +
                      ...
                      (gradT_arrs[nbx](i, j, k, 2) * gravity[2])) *
                    beta_arrs[nbx](i, j, k);
                amrex::Real epsilon = utils::pow(Cmu, 3) *
                                      std::pow(tke_arrs[nbx](i, j, k), 1.5_rt) /
                                      tlscale_arrs[nbx](i, j, k);
                amrex::Real Rt =
                    utils::pow(tke_arrs[nbx](i, j, k) / epsilon, 2) *
