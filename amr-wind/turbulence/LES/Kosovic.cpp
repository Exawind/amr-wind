...
void Kosovic<Transport>::update_turbulent_viscosity(
  ...
  BL_PROFILE("amr-wind::" + this->identifier() +
  ...
  const auto *m_terrain_drag =
      has_terrain ? &this->m_sim.repo().get_int_field("terrain_drag") : nullptr;
  ...
  for (int lev = 0; lev < nlevels; ++lev) {
    ...
    const auto &drag_arrs = has_terrain ? (*m_terrain_drag)(lev).const_arrays()
                                        : amrex::MultiArray4<const int>();
    ...
    const amrex::Real non_neutral_neighbour =
        (m_wall_het_model == "mol")
            ? MOData::calc_psi_m(1.5 * dz / monin_obukhov_length, m_beta_m,
                                 m_gamma_m)
            : 0.0;
    amrex::ParallelFor(mu_turb(lev), [=] AMREX_GPU_DEVICE(int nbx, int i, int j,
                                                          int k) noexcept {
      const amrex::Real rho = rho_arrs[nbx](i, j, k);
      amrex::Real x3 = problo[2] + (k + 0.5) * dz;
      x3 = (has_terrain) ? std::max(x3 - height_arrs[nbx](i, j, k, 0), 0.5 * dz)
                         : x3;
      ...
      const amrex::Real wall_distance =
          (has_terrain)
              ? std::max((k + 1) * dz - height_arrs[nbx](i, j, k, 0), dz)
              : (k + 1) * dz;
      const amrex::Real ransL = std::pow(0.41 * wall_distance / phiM, 2);
      ...
      mu_arrs[nbx](i, j, k) *= rho * viscosityScale * turnOff * blankTerrain;
      // log-law
      ...
      const amrex::Real ustar =
          m * kappa / (std::log(1.5 * dz / local_z0) - non_neutral_neighbour);
      const amrex::Real dUdz =
          0.5 / dz *
          (vel_arrs[nbx](i, j, k + 1, 0) - vel_arrs[nbx](i, j, k - 1, 0));
      const amrex::Real dVdz =
          0.5 / dz *
          (vel_arrs[nbx](i, j, k + 1, 0) - vel_arrs[nbx](i, j, k - 1, 0));
      const amrex::Real dMdz = std::sqrt(dUdz * dUdz + dVdz * dVdz);
      const amrex::Real mut_loglaw = ustar * ustar * rho / (dMdz + 1e-15);
      const amrex::Real drag = (has_terrain) ? drag_arrs[nbx](i, j, k, 0) : 0.0;
      mu_arrs[nbx](i, j, k) =
          mu_arrs[nbx](i, j, k) * (1 - drag) + drag * mut_loglaw;
      amrex::Real stressScale =
          locSurfaceFactor * (std::pow(1 - fmu, locSurfaceRANSExp) *
  ...
  for (int lev = 0; lev < nlevels; ++lev) {
    ...
    amrex::ParallelFor(mu_turb(lev), [=] AMREX_GPU_DEVICE(int nbx, int i, int j,
      ...
      alphaeff_arrs[nbx](i, j, k) =
          lam_diff_arrs[nbx](i, j, k) + muCoeff * muturb_arrs[nbx](i, j, k);
    });
  }
  amrex::Gpu::streamSynchronize();
...
template <typename Transport> void Kosovic<Transport>::parse_model_coeffs() {
