    ...
    , m_divNij(sim.repo().declare_field("divNij",3,1,1))		       
{
  ...
  if(std::abs(m_refMOL)>1000)
    amrex::Print()<<" Length scale assumes Neutral Stratification with Ref MOL:"<<m_refMOL<<std::endl;
  else if(m_refMOL<0)
    amrex::Print()<<" Length scale suggests Unstable  Stratification with Ref MOL:"<<m_refMOL<<std::endl;
  else
    amrex::Print()<<" Length scale suggests Stable  Stratification with Ref MOL::"<<m_refMOL<<std::endl;
  ...
  if(m_surfaceRANS)
    {
      ...
      amrex::Print()<<" Enabling RANS model in the surface layer upto "<<m_switchLoc<<"  m"<<std::endl;
      amrex::Print()<<" Exponential term power coefficient:"<<m_surfaceRANSExp<<std::endl;
    }
  else
    {
      m_surfaceFactor=0;
  ...
  amrex::Print()<<" Turning off SGS model above "<<m_LESTurnOff<<"   m"<<std::endl;
}
    ...
    const FieldState fstate, const DiffusionType /*unused*/)
{
	...
	const amrex::Real ds_sqr = ds * ds;
	    ...
	    const auto& divNijLevel=(this->m_divNij)(lev).array(mfi);
		  ...
		  //if(i==76 && j==76)
		  //amrex::Print()<<i<<"   "<<j<<"  "<<k<<"  "<<x3<<" "<<dz<<std::endl;
		  ...
		  const amrex::Real ransL=std::pow(0.41*(k+1)*dz/phiM,2);
		  amrex::Real turnOff=std::exp(-x3/m_LESTurnOff);
		  //if(x3>=m_LESTurnOff)
		  //  turnOff=0;
		  //const amrex::Real masonScaleInv=1.0/(std::pow(0.41*(k+1)*dz,2))+1/smag_factor;
		  //amrex::Real hybridScale=amrex::min(smag_factor,std::pow(0.41*(k+1)*dz,2));
		  //amrex::Real lscale=m_surfaceFactor*hybridScale+(1-m_surfaceFactor)*smag_factor;
		  amrex::Real lscale=m_surfaceFactor*(std::pow(1-fmu,m_surfaceRANSExp)*smag_factor+std::pow(fmu,m_surfaceRANSExp)*ransL)+(1-m_surfaceFactor)*smag_factor;	   
		  mu_arr(i, j, k) *= rho * lscale*turnOff;
		  //lscale=m_surfaceFactor*hybridScale+(1-m_surfaceFactor)*smag_factor*0.25*m_C1;
		  lscale=m_surfaceFactor*(std::pow(1-fmu,m_surfaceRANSExp)*smag_factor*0.25*m_C1+std::pow(fmu,m_surfaceRANSExp)*ransL)+(1-m_surfaceFactor)*smag_factor*0.25*m_C1;
