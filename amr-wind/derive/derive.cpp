...
Real incflo::ComputeEnstrophy() const
{
    BL_PROFILE("amr-wind::incflo::ComputeEnstrophy");
    ...
    for (int lev = 0; lev <= finest_level; lev++) {
        ...
        Enstrophy += amrex::ReduceSum(
            density()(lev), (*vorticity)(lev), level_mask, 0,
            [=] AMREX_GPU_HOST_DEVICE(
                Box const& bx, Array4<Real const> const& den_arr,
                Array4<Real const> const& vort_arr,
                Array4<int const> const& mask_arr) -> Real {
                Real Enstrophy_Fab = 0.0;
    ...
    ParallelDescriptor::ReduceRealSum(Enstrophy);
