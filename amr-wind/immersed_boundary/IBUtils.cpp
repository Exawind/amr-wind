...
void determine_root_proc(IBInfo& info, amrex::Vector<int>& ib_proc_count)
{
    ...
    if (assigned) {
        ...
        // velocities sampled. Individual immersed boundary instances can override this
        info.sample_vel_in_proc = info.ib_in_proc;
        return;
    }
    ...
    auto it = std::min_element(ib_proc_count.begin(), ib_proc_count.end());
    // Make it the root process for this turbine
    info.root_proc = std::distance(ib_proc_count.begin(), it);
...

    {
        ...
        // By default we request all processes where turbine is active to have
        // velocities sampled. Individual immersed boundaries instances can override this
        info.sample_vel_in_proc = info.ib_in_proc;
    }
}
