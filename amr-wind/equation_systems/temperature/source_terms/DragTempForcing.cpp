    ...
    const amrex::Array4<amrex::Real>& src_term) const
{
  ...
  const auto& temperature = m_temperature.state(field_impl::dof_state(fstate))(lev).const_array(mfi);
    ...
    auto* const m_terrain_blank = &this->m_sim.repo().get_int_field("terrain_blank");
