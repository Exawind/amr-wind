    ...
    const amrex::Array4<amrex::Real>& src_term) const
{
    ...
    for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
        ...
        if (bctype[olo] == BC::wall_model && bx.smallEnd(dir) == geom.Domain().smallEnd(dir)) {
        ...
        if (bctype[ohi] == BC::wall_model && bx.bigEnd(dir) == geom.Domain().bigEnd(dir)) {
            amrex::Box bhi = amrex::bdryHi(bx, dir, 1);
            amrex::Abort("fix me later: need an upper version of lower_boundary_faces");
            if (bhi.ok()) {
                amrex::ParallelFor(
