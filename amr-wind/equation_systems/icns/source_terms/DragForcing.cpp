    ...
    const amrex::Array4<amrex::Real>& src_term) const
{
    ...
    // rho_u_rhs(i, j, k) -= sponge_strength * xi * xi * (rho_u(i, j, k) -
    // sponge_density*sponge_x_velocity);
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
        ...
        src_term(i, j, k, 0) -=
            (CdM * m * ux * blank(i, j, k) + Dxz * drag(i, j, k) +
             (xdamping + ydamping) * (ux - gpu_spongeDensity * gpu_spongeVelX));
        src_term(i, j, k, 1) -=
            (CdM * m * uy * blank(i, j, k) + Dyz * drag(i, j, k) +
             (xdamping + ydamping) * (uy - gpu_spongeDensity * gpu_spongeVelY));
        src_term(i, j, k, 2) -=
            (CdM * m * uz * blank(i, j, k) +
             (xdamping + ydamping) * (uz - gpu_spongeDensity * gpu_spongeVelZ));
    });
}
