    ...
    const amrex::Array4<amrex::Real>& src_term) const
{
    ...
    const amrex::Real vsize=gpu_vel_ht.size();
	...
	xdamping=(1-gpu_verificationMode)*gpu_spongeStrength * xi * xi ;
      }
        if(x2>gpu_startY){
        amrex::Real yi=(x2 - gpu_startY) / (prob_hi[1] - gpu_startY);
	ydamping=(1-gpu_verificationMode)*gpu_spongeStrength * yi * yi;
      }
	  ...
	  if(height_error<residual)
	    {
	      ...
	      gpu_spongeVelX=local_gpu_vel_vals[3*ii];
	      gpu_spongeVelY=local_gpu_vel_vals[3*ii+1];
	      gpu_spongeVelZ=local_gpu_vel_vals[3*ii+2];
	    }
	}
      // Terrain Drag
      amrex::Real Dxz=0.0;
	...
	{ 
	  ...
	  const amrex::Real ustar= (1-gpu_verificationMode)*std::abs(m2-m1)*kappa/std::log((x3+z0)/z0);
	  Dxz= -ustar*ustar*ux1/(1e-5+std::sqrt(ux1*ux1+uy1*uy1))/dx[2];
	  Dyz= -ustar*ustar*uy1/(1e-5+std::sqrt(ux1*ux1+uy1*uy1))/dx[2];
	}
      // Adjusting Cd for momentum
      amrex::Real CdM=std::min(Cd/(m+1e-5),1000.0);
      src_term(i, j, k, 0) -= (CdM*m*ux1*blank(i,j,k)+Dxz*drag(i,j,k)+(xdamping+ydamping)*(ux1-gpu_spongeDensity*gpu_spongeVelX));
      src_term(i, j, k, 1) -= (CdM*m*uy1*blank(i,j,k)+Dyz*drag(i,j,k)+(xdamping+ydamping)*(uy1-gpu_spongeDensity*gpu_spongeVelY));
      src_term(i, j, k, 2) -= (CdM*m*uz1*blank(i,j,k)+(xdamping+ydamping)*(uz1-gpu_spongeDensity*gpu_spongeVelZ));
    });
}
