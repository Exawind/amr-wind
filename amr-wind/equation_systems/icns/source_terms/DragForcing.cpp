    ...
    const amrex::Array4<amrex::Real>& src_term) const
{
    ...
    const amrex::Real Cd = (is_laminar && dx[2]<1)? dragCoefficient:dragCoefficient / dx[2];
	...
	amrex::Real bcforcing_y=0; 
        ...
        if (drag(i,j,k) == 1 && (! is_laminar)) {
            ...
            const amrex::Real ustar = m2 * kappa /
                                      std::log(1.5*dx[2] / z0);
	    const amrex::Real uTarget= ustar/kappa*std::log(0.5*dx[2]/z0);
	    const amrex::Real uxTarget = uTarget * ux2 /
              (1e-5 + std::sqrt(ux2 * ux2 + uy2 * uy2));
	    const amrex::Real uyTarget = uTarget * uy2 /
              (1e-5 + std::sqrt(ux2 * ux2 + uy2 * uy2));
        ...
        const amrex::Real CdM = std::min(Cd / (m + 1e-5), 1000.0/scale_factor);
        src_term(i, j, k, 0) -=
	  (CdM * m * ux1 * blank(i, j, k) + Dxz * drag(i, j, k) + bcforcing_x * drag(i,j,k) +
             (xstart_damping + xend_damping+ ystart_damping+yend_damping) * (ux1 - sponge_density * spongeVelX));
        src_term(i, j, k, 1) -=
	  (CdM * m * uy1 * blank(i, j, k) + Dyz * drag(i, j, k) + bcforcing_y * drag(i,j,k) +
             (xstart_damping + xend_damping+ ystart_damping+yend_damping) * (uy1 - sponge_density * spongeVelY));
        src_term(i, j, k, 2) -=
            (CdM * m * uz1 * blank(i, j, k) +
             (xstart_damping + xend_damping+ ystart_damping+yend_damping) * (uz1 - sponge_density * spongeVelZ));
    });
}
