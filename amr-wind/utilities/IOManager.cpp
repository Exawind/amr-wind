...
//    fill_val_x[i] = problo[0] +
//    dx[0]*(static_cast<amrex::Real>(i)+0.5)*factormap[0];  // perform mapping
//    to the non-uniform space
//  }
//  xcell.put(fill_val_x.data());
//
//  for (int j = dlo.y; j<= dhi.y; j++) {
//    fill_val_y[j] = problo[1] +
//    dx[1]*(static_cast<amrex::Real>(j)+0.5)*factormap[1];  // perform mapping
//    to the non-uniform space
//  }
//  ycell.put(fill_val_y.data());
//  for (int k = dlo.z; k<= dhi.z; k++) {
//    fill_val_z[k] = problo[2] +
//    dx[2]*(static_cast<amrex::Real>(k)+0.5)*factormap[2];  // perform mapping
//    to the non-uniform space
//  }
...
//
//    amrex::Vector<size_t> start = {static_cast<unsigned long>(lo.x),
//    static_cast<unsigned long>(lo.y), static_cast<unsigned long>(lo.z)};
//    amrex::Vector<size_t> count = {static_cast<unsigned long>(hi.x-lo.x+1),
//    static_cast<unsigned long>(hi.y-lo.y+1), static_cast<unsigned
//    long>(hi.z-lo.z+1)}; xvelncf.put(xvel.dataPtr(), start, count);
