...
void PlaneAveraging::plot_line_text(std::string filename, int step, Real time)
{
    ...
    if (step == 1) {
        ...
        outfile << "#step,time,z,u_avg,v_avg,w_avg,T_avg,nu_avg,uu,uv,uw,vv,vw,"
                   "ww,wuu,wuv,wuw,wvv,wvw,www,Tu,Tv,Tw"
                << std::endl;
    } else {
        // append file
    ...
    for (int i = 0; i < ncell_line; ++i) {
        ...
        for (int n = 0; n < nfluc; ++n) {
            outfile << ", " << std::scientific
                    << line_fluctuation[nfluc * i + n];
        }
        outfile << std::endl;
    }
}

void PlaneAveraging::plot_line_average_text(
...
void PlaneAveraging::plot_line_binary(std::string filename, int step, Real time)
{
    ...
    if (step == 1) {
        ...
        outfile.write(
            (char*)line_xcentroid.data(), sizeof(Real) * line_xcentroid.size());
    ...
    outfile.write(
        (char*)line_average.data(), sizeof(Real) * line_average.size());
    outfile.write(
        (char*)line_fluctuation.data(), sizeof(Real) * line_fluctuation.size());
}

template <typename IndexSelector>
void PlaneAveraging::fill_line(
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
    ...
    ParallelDescriptor::ReduceRealSum(
        line_fluctuation.data(), line_fluctuation.size());
}
...
Real PlaneAveraging::eval_line_average(Real x, int comp) const
{
    ...
    return line_average[navg * ind + comp] * (1.0 - c) +
           line_average[navg * (ind + 1) + comp] * c;
}

PlaneAveraging::PlaneAveraging(int axis_in) : axis(axis_in)
{
    AMREX_ALWAYS_ASSERT(axis >= 0 and axis <= 2);
}

void PlaneAveraging::operator()(
    ...
    amrex::Vector<amrex::MultiFab*> const& temperature)
{
    ...
    // level=0 is default, could later make this an input. Might only makes
    // sense for fully covered levels
    ...
    case 0:
        fill_line(
            XDir(), *density[level], *velocity[level], *mueff[level],
            *temperature[level]);
        break;
    case 1:
        fill_line(
            YDir(), *density[level], *velocity[level], *mueff[level],
            *temperature[level]);
        break;
    case 2:
        fill_line(
            ZDir(), *density[level], *velocity[level], *mueff[level],
            *temperature[level]);
        break;
    default:
