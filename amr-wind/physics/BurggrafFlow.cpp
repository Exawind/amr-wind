...
{
    return 8.0_rt *
           (amr_wind::utils::pow(x, 4) - 2.0_rt * amr_wind::utils::pow(x, 3) +
            amr_wind::utils::pow(x, 2)) *
           (4.0_rt * amr_wind::utils::pow(y, 3) - 2.0_rt * y);
}
...
{
    return -8.0_rt *
           (4.0_rt * amr_wind::utils::pow(x, 3) -
            6.0_rt * amr_wind::utils::pow(x, 2) + 2.0_rt * x) *
           (amr_wind::utils::pow(y, 4) - amr_wind::utils::pow(y, 2));
}
...
void BurggrafFlow::initialize_fields(int level, const amrex::Geometry& geom)
{
    ...
    amrex::ParallelFor(
        velocity, [=] AMREX_GPU_DEVICE(int nbx, int i, int j, int k) {
            ...
            const amrex::Real f = amr_wind::utils::pow(x, 4) -
                                  (2.0_rt * amr_wind::utils::pow(x, 3.0_rt)) +
                                  amr_wind::utils::pow(x, 2);
            const amrex::Real f1 = (4.0_rt * amr_wind::utils::pow(x, 3.0_rt)) -
                                   (6.0_rt * amr_wind::utils::pow(x, 2)) +
                                   (2.0_rt * x);
            const amrex::Real f3 = (24.0_rt * x) - 12.0_rt;
            const amrex::Real g =
                amr_wind::utils::pow(y, 4) - amr_wind::utils::pow(y, 2);
            const amrex::Real g1 =
                (4.0_rt * amr_wind::utils::pow(y, 3.0_rt)) - (2.0_rt * y);
            const amrex::Real g2 =
                (12.0_rt * amr_wind::utils::pow(y, 2)) - 2.0_rt;

            const amrex::Real F = (amr_wind::utils::pow(x, 5.0_rt) / 5.0_rt) -
                                  (amr_wind::utils::pow(x, 4) / 2.0_rt) +
                                  (amr_wind::utils::pow(x, 3.0_rt) / 3.0_rt);
            const amrex::Real F1 = (-4.0_rt * amr_wind::utils::pow(x, 6.0_rt)) +
                                   (12.0_rt * amr_wind::utils::pow(x, 5.0_rt)) -
                                   (14.0_rt * amr_wind::utils::pow(x, 4)) +
                                   (8.0_rt * amr_wind::utils::pow(x, 3.0_rt)) -
                                   (2.0_rt * amr_wind::utils::pow(x, 2));
            const amrex::Real F2 = f * f / 2.0_rt;
            const amrex::Real G1 =
                (-24.0_rt * amr_wind::utils::pow(y, 5.0_rt)) +
                (8.0_rt * amr_wind::utils::pow(y, 3.0_rt)) - (4.0_rt * y);
