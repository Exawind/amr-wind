...
HybridRANSLESABL::HybridRANSLESABL(CFDSim& sim)
    : m_repo(sim.repo())
    , m_tke(sim.repo().get_field("tke"))
    , m_sdr(sim.repo().declare_field("sdr", 1, m_tke.num_grow()[0], 1))
{
    AMREX_ALWAYS_ASSERT(sim.turbulence_model().model_name() == "OneEqKsgsM84");
    auto t_coeffs = sim.turbulence_model().model_coeffs();
    m_Ce = t_coeffs["Ce"];
}
...
void HybridRANSLESABL::compute_sdr()
{

    BL_PROFILE("amr-wind::" + this->identifier() + "::compute_sdr");

    auto& tke = this->m_tke;
    auto& sdr = this->m_sdr;
    const amrex::Real Ce = this->m_Ce;

    auto& geom_vec = m_repo.mesh().Geom();
