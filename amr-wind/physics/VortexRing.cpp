    ...
    for (amrex::MFIter mfi(velocity); mfi.isValid(); ++mfi) {
        ...
        amrex::ParallelFor(
            nbx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
				const amrex::Real m_R = 1.0;
				const amrex::Real m_Gamma = 1.0;
				const amrex::Real m_alpha = 0.54857674;
				...
				const amrex::Real ssq = std::pow(z, 2) + std::pow(r - m_R, 2);
				const amrex::Real s = std::sqrt(ssq);
                vort(i, j, k, 0) = -std::sin(theta) * vorticity_theta(s, ssq, m_R, m_Gamma, m_alpha);
                vort(i, j, k, 1) = std::cos(theta) * vorticity_theta(s, ssq, m_R, m_Gamma, m_alpha);
                vort(i, j, k, 2) = 0.0;
            });
    }
