#include "amr-wind/physics/FatCoredVortexRing.H"
...
{
    if (s <= R) {
        return alpha * Gamma / std::pow(R, 2) *
               std::exp(-4 * ssq / (std::pow(R, 2) - ssq));
    } else {
        return 0.0;
    }
}

FatCoredVortexRing::FatCoredVortexRing(const CFDSim& sim)
    : m_repo(sim.repo())
    ...
    , m_density(sim.repo().get_field("density"))
{
    ...
    {
        amrex::ParmParse pp("fatcoredvortexring");
        pp.query("R", m_R);
 ...
 *  simulation.
 */
void FatCoredVortexRing::initialize_fields(
    int level, const amrex::Geometry& geom)
{
    ...
    for (amrex::MFIter mfi(velocity); mfi.isValid(); ++mfi) {
        ...
        amrex::ParallelFor(
            nbx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                ...
                minvort(i, j, k, 0) =
                    std::sin(theta) * vorticity_theta(s, ssq, R, Gamma, alpha);
                minvort(i, j, k, 1) =
                    -std::cos(theta) * vorticity_theta(s, ssq, R, Gamma, alpha);
                minvort(i, j, k, 2) = 0.0;
            });
    }
    ...
    amrex::MLNodeLaplacian linop(
        mesh.Geom(0, level), mesh.boxArray(0, level),
        mesh.DistributionMap(0, level), info, {}, 1.0);
