  ...
  void TerrainDrag::post_init_actions()
{
  ...
  for (int level = 0; level < nlevels; ++level) {
    ...
    amrex::Gpu::copy(amrex::Gpu::hostToDevice, m_xterrain.begin(), m_xterrain.end(), gpu_xterrain.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice, m_yterrain.begin(), m_yterrain.end(), gpu_yterrain.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice, m_zterrain.begin(), m_zterrain.end(), gpu_zterrain.begin());
    ...
    amrex::Gpu::copy(amrex::Gpu::hostToDevice, m_xrough.begin(), m_xrough.end(), gpu_xrough.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice, m_yrough.begin(), m_yrough.end(), gpu_yrough.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice, m_z0rough.begin(), m_z0rough.end(), gpu_z0rough.begin());
	...
	const amrex::Real roughnessSize=m_xrough.size();
        amrex::ParallelFor(
		...
		for(int ii=0;ii<terrainSize;++ii)
		  {
		    const amrex::Real radius=std::sqrt(std::pow(x1-xterrain_ptr[ii],2)+std::pow(x2-yterrain_ptr[ii],2));
		    if(radius<residual){
		      residual=radius;
		...
		for(int ii=0;ii<roughnessSize;++ii)
		  {
		    const amrex::Real radius=std::sqrt(std::pow(x1-xrough_ptr[ii],2)+std::pow(x2-yrough_ptr[ii],2));
		    if(radius<residual){
		      residual=radius;
	...
	amrex::ParallelFor(vbx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
	  ...
	  if(x3>terrainHt && k>0 && levelBlanking(i,j,k-1,0)==1){
