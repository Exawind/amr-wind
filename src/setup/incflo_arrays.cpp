...
void incflo::RegridArrays(int lev)
{
   ...
   // Tracer
   std::unique_ptr<MultiFab> tracer_new(new MultiFab(grids[lev], dmap[lev], ntrac, nghost, MFInfo()));
   tracer_new->setVal(0.0);
   tracer_new->copy(*tracer[lev], 0, 0, ntrac, 0, nghost);
   tracer[lev] = std::move(tracer_new);

   // Old Tracer
   std::unique_ptr<MultiFab> tracer_o_new(new MultiFab(grids[lev], dmap[lev], ntrac, nghost, MFInfo()));
   tracer_o_new->setVal(0.0);
   tracer_o_new->copy(*tracer_o[lev], 0, 0, ntrac, 0, nghost);
   tracer_o[lev] = std::move(tracer_o_new);
    ...
    ****************************************************************************/
    ...
    // MAC velocity
    std::unique_ptr<MultiFab> u_mac_new(new MultiFab(x_ba, dmap[lev], 1, nghost, MFInfo()));
    m_u_mac[lev] = std::move(u_mac_new);
    m_u_mac[lev]->setVal(0.0);
    ...
    // MAC velocity
    std::unique_ptr<MultiFab> v_mac_new(new MultiFab(y_ba, dmap[lev], 1, nghost, MFInfo()));
    m_v_mac[lev] = std::move(v_mac_new);
    m_v_mac[lev] -> setVal(0.0);
    ...
    // MAC velocity
    std::unique_ptr<MultiFab> w_mac_new(new MultiFab(z_ba, dmap[lev], 1, nghost, MFInfo()));
    m_w_mac[lev] = std::move(w_mac_new);
    m_w_mac[lev] -> setVal(0.0);
#endif
}
