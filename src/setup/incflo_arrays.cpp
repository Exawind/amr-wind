...
void incflo::AllocateArrays(int lev)
{
    ...
    // Tracer Viscosity
    eta_tracer[lev].reset(new MultiFab(grids[lev], dmap[lev], ntrac, nghost, MFInfo(), factory));
...
void incflo::RegridArrays(int lev)
{
   ...
   vel[lev] = std::move(vel_new);
    ...
    std::unique_ptr<MultiFab> eta_tracer_new(new MultiFab(grids[lev], dmap[lev], ntrac, nghost, MFInfo(), factory));
