...
contains
    ...
    subroutine init_fluid(slo, shi, lo, hi, &
        ...
        if (probtype == 36) call rayleigh_benard(lo, hi, vel, tracer, slo, shi, dx, dy, dz, xLo, yLo, zLo, domlo)

    end subroutine init_fluid

    subroutine tracer_advection(lo, hi, vel, tracer, slo, shi, dx, dy, dz, domlo)
        ...
        ! Arrays
        real(rt),       intent(inout) ::    vel(slo(1):shi(1),slo(2):shi(2),slo(3):shi(3),3)
        real(rt),       intent(inout) :: tracer(slo(1):shi(1),slo(2):shi(2),slo(3):shi(3),3)
        ...
        do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
                ...
                do k = lo(3), hi(3)
                    tracer(i,j,k,:) = 1.d0
                    if (i.eq.20 .and. j.eq.20 .and. k.eq.20) tracer(i,j,k,1) = 1.2d0
                end do
            end do
        end do
    ...
    ! Initialize potential temperature with a perturbation factor
    ! RBC domain should have height of unity and be solved with non-dimensional variables
    subroutine rayleigh_benard(lo, hi, vel, tracer, slo, shi, dx, dy, dz, xLo, yLo, zLo, domlo)
        use constant, only: ntrac, zero, one
        implicit none
        ...
        integer(c_int)                :: i, j, k
        real(rt)                      :: x, y, z, perturb, pi

        pi = acos(-1.0)

        if(ntrac .ne. 1) then
            print *, "ntrac here is", ntrac
            print *, "probtype = 36 is designed for 1 tracer"
            stop
        end if
        ...
        do k = lo(3), hi(3)
            ...
            do j = lo(2), hi(2)
                ...
                do i = lo(1), hi(1)
                    ...
                    vel(i,j,k,2) = zero
                    vel(i,j,k,3) = zero
                    perturb = (sin(5.0*pi*x) + cos(5.0*pi*y))*1.0e-3
                    tracer(i,j,k,1) = - z + perturb
                 end do
            end do
        end do
    ...
    end subroutine rayleigh_benard
