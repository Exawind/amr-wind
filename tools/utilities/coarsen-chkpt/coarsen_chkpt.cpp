        ...
        argc, argv, true, amrex::mpidatatypes::MPI_COMM_WORLD, []() {
            {
                ...
                // attempting to generate backtrace files. However, if the user
                // has explicitly set these options in their input files respect
                // those settings.
            ...
            }
            {
                ...
                // Getting the right number of levels is important for geometry
                // init
                ...
                if ((ncell[0] % 2) % 8 != 0 || (ncell[1] % 2) % 8 != 0 ||
                    (ncell[2] % 2) % 8 != 0) {
                    amrex::Abort(
                        "Too few cells to coarsen with default blocking "
                        "factor");
                }
                // Make sure that target matches
                if ((ncell[0] / 2) != tncell[0] ||
                    (ncell[1] / 2) != tncell[1] ||
                    (ncell[2] / 2) != tncell[2]) {
